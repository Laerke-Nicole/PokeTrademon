<template>
  <div class="pt-30 five-percent dark-bg min-h-screen">
    ...
    <div class="max-w-screen-xl mx-auto px-4">
      <h1
        class="text-3xl text-center dark-headline pb-10 text-center w-full"
          ...
          <div class="grid md:grid-cols-2 gap-6 mt-6">
            ...
            >
              <div class="flex justify-between pb-2">
                ...
                <span
                  class="text-xs px-2 py-1 rounded-full font-semibold bg-yellow-100 text-yellow-800"
                >
                  Open Offer
                </span>
              </div>
              ...
              <p class="text-sm dark-text">
                <strong>They offer:</strong> {{ cardList(offer.senderCards) }}
              </p>
              <p class="text-sm dark-text">
                <strong>They want:</strong> {{ cardList(offer.receiverCards) }}
              </p>
            ...
            >
              <div class="flex justify-between pb-2">
                <div class="font-semibold dark-text">
                  From: {{ trade.senderId.username.trim() }}
                </div>
                <span
                  :class="[
              ...
              <p class="text-sm dark-text">
                <strong>They offer:</strong> {{ cardList(trade.senderCards).trim() }}
              </p>
              <p class="text-sm dark-text">
                <strong>They want:</strong> {{ cardList(trade.receiverCards).trim() }}
              </p>
              <div v-if="trade.status === 'pending'" class="pt-4 flex space-x-2">
                <button
                  class="bg-green-500 hover:bg-green-600 light-headline px-4 py-1 rounded-corner text-sm"
                  @click="acceptTrade(trade._id)"
                >
                  Accept
                </button>
                <button
                  class="bg-red-500 hover:bg-red-600 light-headline px-4 py-1 rounded-corner text-sm"
                  @click="declineTrade(trade._id)"
                >
                  Decline
                </button>
              </div>
        ...
        <div v-else-if="activeTab === 'outgoing'">
          ...
          <div class="grid md:grid-cols-2 gap-6">
            ...
            >
              <div class="flex justify-between pb-2">
                <div class="font-semibold dark-text">
                  To: {{ trade.receiverId?.username || 'Anyone (Open)'.trim() }}
                </div>
                <span
                  :class="[
                    'text-xs px-2 py-1 rounded-full font-semibold',
              ...
              <p class="text-sm dark-text">
                <strong>You offer:</strong> {{ cardList(trade.senderCards).trim() }}
              </p>
              <p class="text-sm dark-text">
                <strong>You want:</strong> {{ cardList(trade.receiverCards).trim() }}
              </p>
            </div>
          </div>
        </div>
        ...
        <div v-else-if="activeTab === 'completed'">
          ...
          <div class="grid md:grid-cols-2 gap-6">
            ...
            >
              <div class="flex justify-between pb-2">
                <div class="font-semibold dark-text">
                  <span v-if="isSender(trade)"> To: {{ getUsername(trade.receiverId) }} </span>
                  <span v-else> From: {{ getUsername(trade.senderId) }} </span>
                </div>
                <span
                  :class="[
                    'text-xs px-2 py-1 rounded-full font-semibold',
...
const incoming = computed(() =>
  allTrades.value.filter((t) => {
    ...
    return receiver === userId.value && t.status === 'pending'
  }),
)
...
const outgoing = computed(() =>
  allTrades.value.filter((t) => {
    ...
    return sender === userId.value && t.status === 'pending'
  }),
)
...
const completed = computed(() =>
  allTrades.value.filter((t) => {
    const iAmSender =
      (typeof t.senderId === 'string' ? t.senderId : t.senderId?._id) === userId.value
    const iAmReceiver =
      (typeof t.receiverId === 'string' ? t.receiverId : t.receiverId?._id) === userId.value
    return (iAmSender || iAmReceiver) && (t.status === 'accepted' || t.status === 'declined')
  }),
)
...
const getCardMax = (cardId: string): number =>
  userCollection.value.find((c) => c.cardId === cardId)?.quantity ?? 1
