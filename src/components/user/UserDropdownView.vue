<template>
  <div
    v-if="state"
    class="dropdown absolute right-0 mt-2 w-40 bg-white rounded-lg shadow-lg border z-10 p-2"
  >
    <ul class="text-sm text-gray-700 flex flex-col w-full">
      ...
      <div v-if="state.isLoggedIn">
        <li v-if="user" class="w-full px-4 py-2">
          Welcome back <span class="user-name">{{ user.username.trim() }}!</span>
        </li>
      ...
      <div>
        ...
        <li
          v-else-if="readyToShowLinks && isUser === null"
          class="w-full px-4 py-2 italic text-gray-500"
        >
      ...
      <div>
        ...
        <li
          v-else-if="readyToShowLinks && isAdmin === null"
          class="w-full px-4 py-2 italic text-gray-500"
        >
          Loading admin page...
        </li>
      ...
      <button
        ...
        @click="logout"
        class="hover:bg-gray-100 w-full text-left px-4 py-2 underline cursor-pointer"
      >
        Log out
      </button>
    </ul>
